...
public class NearestFirstParkingStrategy implements Strategy {
    ...
    public ParkingSpot findParkingSpot(ParkingSpotEnum parkingSpotEnum) throws SpotNotFoundException {
        ...
        NavigableSet<ParkingSpot> freeSpots = ParkingLot.getInstance().getFreeparkingSpots().get(parkingSpotEnum);

        if (freeSpots.isEmpty()) {
            throw new SpotNotFoundException(
                    "Spot not found in Nearest First Strategy");
        }

        return freeSpots.first();  // O(log n)
    }
}
